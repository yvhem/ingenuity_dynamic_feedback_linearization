...
\begin{align*} 
    ...
    \end{pmatrix} + \begin{pmatrix}
        0 \\ 0 \\ -g
    \end{pmatrix} - \frac{1}{m}A_\text{trans}V^b
\end{align*}
...
\end{equation}
where $\bs{l}(\bs\xi, \bs\zeta)$ is the drift vector containing all state-dependent terms, and $\bs{J}(\bs\xi,\bs\zeta)$ is the $4 \times 4$ decoupling matrix. The linearizing control law is chosen to cancel these terms:
...
\begin{equation}\label{eq:position_controller}
    v_{1,2,3} = \bs{P}_d^{(4)} + K_3 \bs{e}_P^{(3)} + K_2 \ddbs{e}_P + k_1 \dbs{e}_P + K_0 \bs{e}_P
\end{equation}
where $v_{1,2,3} = (v_1, v_2, v_3)^T$ and $K_i = \text{diag}(k_{i,x}, k_{i,y}, k_{i,z})$ are diagonal gain matrices.

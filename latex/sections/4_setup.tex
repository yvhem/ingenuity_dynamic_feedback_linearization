...
The simulation is implemented in MATLAB. Since the system dynamics derived in Chapter \ref{ch:Dynamics} are nonlinear and continuous in time, we use a standard numerical solver for ordinary differential equations (\texttt{ode15s}) to integrate the system dynamics. The complete source code for the project, including the dynamic model and the controller implementation, is available on GitHub\footnote{\url{https://github.com/yvhem/ingenuity_dynamic_feedback_linearization/tree/main}}.
